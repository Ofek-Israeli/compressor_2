...
def load_config(config_path: str, validate_ga: bool = True) -> EvolutionConfig:
    ...
    cfg = EvolutionConfig(
        ...
        validation_indices=_parse_pool_indices(str(v.get("VALIDATION_INDICES", ""))) if str(v.get("VALIDATION_INDICES", "")).strip() else None,
        validation_top_k=int(v.get("VALIDATION_TOP_K", 5)),
