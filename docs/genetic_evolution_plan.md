...
This document describes the **current implementation** of the evolution loop in compressor_2: a **DEAP**-based genetic evolution of steering deltas. It is the **only** evolution mode (no legacy or alternative paths). The description assumes the workflow **always runs on a 2-GPU pod** (embedding on one GPU, SGLang on the other; see §5 and `docs/2XGPU_pod_plan.md`). Fitness is computed on a **minibatch** sampled from the pool each generation; mutation is reflector-based; crossover is weighted average of deltas by fitness and reflector merge of scratchpads.
...
- **Evolution loop (high level):**
  ...
  4. **Generation loop (as implemented):**
     - **Offspring count:** `population_size - elitism_size`; next population = elites + offspring = **population_size**.
     - **(0) Sample minibatch** from pool; invalidate all fitness.
     - **(1) Evaluate population:** If all individuals share the same genome (e.g. initial clones), evaluate once and assign the same fitness and training_results to all. Otherwise, for each individual `i`: wait for prefetch (if any), **submit prefetch for individual i+1** (so processor generation runs in parallel with the current eval), then run **evaluate(i)** (generate processor or use prefetched path, run training set via SGLang, run correctness in parallel via ThreadPoolExecutor, compute fitness).
     - **Elitism:** Select top `CONFIG_ELITISM_SIZE` elites from the evaluated population and clone them into the next population.
     - **Update best-ever** from current population.
     - **(2) Selection:** Select `2 × (population_size − elitism_size)` parents (truncation or tournament per config).
     - **(3) Crossover:** Build offspring list of size `population_size − elitism_size`: for each parent pair, with probability CONFIG_CXPB apply weighted-average crossover + reflector merge for scratchpad; otherwise clone one parent at random (50/50). **Pre-queue prefetch for offspring[0]** (single in-flight prefetch thread).
     - **(4) Evaluate offspring:** For each child `j`: wait for prefetch, **submit prefetch for offspring j+1**, then evaluate(child). With probability CONFIG_MUTPB, call reflector for mutation using the child's training results, apply new deltas and scratchpad in-place, then **re-evaluate** the mutated child (no prefetch; inline generate-processor). Update best-ever from offspring.
     - **(5) Replace:** Next population = elite clones + offspring. **Pre-queue prefetch for pop[0]** of the next generation (runs during persist/logging).
     - **Persistence:** Save `evolution_state.json`, `ga_history.json`, `evolution_tree.json`; compile evolution tree to TeX/PDF/**PNG** and maintain **evolution_tree.png** in the output directory (e.g. `output_dir/evolution_tree.png`); save **ga_fitness.png** (fitness vs generation plot, e.g. in the same output directory); save best-ever and per-generation deltas to `deltas_best.json` and `deltas_gen_XXX.json`. Processor .py files are not generated during evolution (run `compressor_2 generate-evolution-processors --config .config` after evolution to produce `processor_best.py`, `processor_gen_XXX.py`, etc.). **Incremental save and Ctrl+C:** State is written at the end of each generation and on **SIGINT (Ctrl+C)**; if the user interrupts with Ctrl+C and runs again, the run **resumes from the same place** (load `evolution_state.json`, re-evaluate population, then continue). Persist and exit on Ctrl+C; resume re-evaluates the population and continues (§2).
  5. **SGLang lifecycle:** SGLang is started **once** before the generation loop and kept running. The logit processor is sent per-request by the runner client (via `custom_logit_processor` in the JSON payload), so the same server serves any processor file without restart. For each individual: generate-processor (or use prefetched result), run training set via the already-running server, run correctness (parallelised). If SGLang crashes, auto-restart on next evaluation.
...
- **LLM reply token budget:** **CONFIG_LLM_MAX_TOKENS** (int): maximum number of tokens the LLM may generate per reply to a query. Passed into the runner config (e.g. SGLang/generation section) so that GA and zero-order runs cap reply length. Required or sensible default (e.g. 2048); loader validates > 0. Shared with zero-order path (see `docs/zero_order_opt_plan.md` §1.5).
...
- **Mandatory exactly 2 GPUs (2-GPU pod).** Evolution is run on a **two-GPU pod**; it requires **exactly 2** visible GPUs (distinct). At startup, the GA driver validates visibility via `_validate_2_gpus()` and **fails fast** if not exactly 2 (see `docs/2XGPU_pod_plan.md` for the precise error messages). If the host has more than 2 GPUs, set `CUDA_VISIBLE_DEVICES` to exactly two indices before running.
- **Assignment:** Embedding is on GPU-A (`cuda:0`), SGLang is on GPU-B (`cuda:1`). Subprocesses inherit the parent's visible set (no `CUDA_VISIBLE_DEVICES` override). Pin devices via framework/device selection: generate-processor and embedder use `cuda:0` (via `--device cuda:0`); SGLang uses `cuda:1` via a CUDA_VISIBLE_DEVICES shim in `SGLangServer.start()` (because SGLang does not support a `--device` flag).
- **SGLang lifecycle:** Started **once** before the generation loop, **kept running** across all evaluations. The logit processor is sent per-request by the runner client (`custom_logit_processor` in JSON payload), so no restart is needed when switching processors. Auto-restart on crash via `is_running()` check.
- **Prefetch:** Always on. While evaluating individual i (SGLang + correctness on `cuda:1`), generate-processor for individual i+1 runs on `cuda:0`. Pre-queue prefetch for offspring[0] after crossover; pre-queue prefetch for pop[0] at end of each generation (overlaps with persist/logging phase). Use non-clashing paths (monotonic counter).
- **Correctness:** Parallelised across examples with `ThreadPoolExecutor` (OpenAI API calls, no GPU).
- **Runtime verification:** After the single SGLang start (before gen loop), verify embedding is on `cuda:0` and SGLang's PID is on the expected GPU (visible GPU #1) by UUID match via NVML or nvidia-smi fallback. Fail fast if mismatched.

---

## 6. Implementation Summary (current state)

The following reflects the **current implementation** in the repo (not a to-do list). Evolution always runs on a 2-GPU pod.
...
4. **Fitness:** `evaluate_individual(...)`: generate processor (or use prefetched), run minibatch via the already-running SGLang server, run correctness (parallelised), return (shortness_score, correctness_ratio); cache minibatch results for reflector. Fitness = lambda_shortness * shortness + lambda_correctness * correctness. DEAP single-objective maximization.
...
9. **Logging and graphs:** Best/mean fitness per generation; fitness vs generation plot saved as **ga_fitness.png** (e.g. in output_dir); **best-ever** deltas saved to `deltas_best.json` (and per-generation best to `deltas_gen_XXX.json`); run **`generate-evolution-processors`** after evolution to produce `processor_best.py` and `processor_gen_XXX.py`. **Reflector prompts:** §8. **Evolution tree:** Maintain **evolution_tree.json** and compile to TeX/PDF/PNG; keep **evolution_tree.png** updated in the output directory (e.g. after each generation or at persist time). Parent–child graph across generations (§8).
...
- **Resume** (§2): `evolution_state.json`; generation, population, **best_individual** (best-ever / global best), best_fitness (optional but recommended), re-evaluate on resume then recompute global best. **Incremental save:** State is written at end of each generation and on SIGINT (Ctrl+C) so that interrupting and re-running **starts from the same place**.
- **Canonical order** (§1): `sorted(expected_cluster_ids, key=int)`.
- **Reflector prompts** (§8): log exact system+user messages per call.
- **GPU** (§5): Evolution runs on a **2-GPU pod**; exactly 2 visible GPUs required; embedding on GPU-A (`cuda:0`), SGLang on GPU-B (`cuda:1`); SGLang started once and kept running (no per-individual restart); always-on prefetch with cross-gen and offspring[0] pre-queuing; correctness parallelised via ThreadPoolExecutor; runtime verification by GPU UUID after SGLang start.

---

## 8. Implementation Notes (reference)

These notes describe behaviour and conventions used in the current implementation. **2-GPU pod:** There is no single-GPU or configurable-GPU-id mode; evolution always assumes exactly two visible GPUs as in §5.
