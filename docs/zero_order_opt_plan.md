...
- In `kconfig_loader.py`, add parsing for the new symbols. When `CONFIG_OPTIMIZATION_METHOD` is present and not `"deap"`, set `cfg.optimization_method`, `cfg.zero_order_max_evals`, `cfg.optimizer_seed`, `cfg.deltas_bound_low`, `cfg.deltas_bound_high`, `cfg.eval_set_mode`, `cfg.eval_minibatch_size` (when applicable), `cfg.eval_seed`, `cfg.eval_timeout_s`, `cfg.eval_max_retries`, `cfg.eval_failure_fitness`, `cfg.cache_round_decimals` or `cfg.cache_eps`, `cfg.enable_cache`, `cfg.eval_deterministic`, `cfg.inference_seed` or `cfg.sglang_seed`, `cfg.run_final_full_pool_eval`, `cfg.final_eval_set_mode` (optional), `cfg.use_multi_fidelity`, and when multi-fidelity: `cfg.search_eval_set_mode`, `cfg.search_minibatch_size`, `cfg.confirm_eval_set_mode`, `cfg.confirm_minibatch_size`, `cfg.confirm_every_k_evals`, `cfg.confirm_improvement_eps`, etc. Call `_validate_ga_options(cfg)` only when `optimization_method == "deap"`.
...
**Multi-fidelity (optional):** When CONFIG_USE_MULTI_FIDELITY=y, search uses a cheap objective (e.g. fixed minibatch) and confirmation uses an expensive one (e.g. full_pool). Multi-fidelity is **especially recommended** for **random search**, **differential evolution**, **CMA-ES**, **PSO**, and **surrogate methods** (skopt, SMAC, TPE/Optuna, RBFOpt, DIRECT) to reduce overfitting to small minibatches. The feature is **explicitly optional** (default off); the current plan remains valid without it.
...
5. **Dispatcher:** Implement `evolution/optimize_driver.py` to dispatch by cfg.optimization_method, run optimizer, save deltas_best.json, **zero_order_state.json** (with n_evals_used, max_evals, remaining_evals, best_x, best_f, best_eval_id, optimizer_seed, eval_seed, eval_set_mode, indices_hash, history_path, final_full_pool_f when applicable; when **CONFIG_USE_MULTI_FIDELITY=y** add best_search_x/f, best_confirm_x/f, confirm_schedule—see §9), **evaluation_indices.json** when fixed_minibatch (and persist pool/hash for full_pool), **final_eval.json** when CONFIG_RUN_FINAL_FULL_POOL_EVAL=y (point = best_x or **best_confirm_x** when multi-fidelity; f_full_pool, shortness_full_pool, correctness_full_pool, indices_hash_full_pool—see §9.2), and optionally processor_best.py once at end. **Final stage:** after optimizer returns, run final eval on full_pool (point = best_x, or **best_confirm_x** when multi-fidelity); persist final_eval.json and final_full_pool_f in state; do not overwrite best_f. **Resume:** support loading zero_order_state.json and continuing (Optuna/SMAC via native persistence; others: x0=best_x, remaining_evals, same or fallback method—see §9.1). **Temp artifacts:** use eval_id in filenames and keep only last N (see §1.2).
...
- **Multi-fidelity (optional, default off):** CONFIG_USE_MULTI_FIDELITY=n by default. When y: search eval (cheap) + confirm eval (expensive); CONFIG_SEARCH_EVAL_SET_MODE, CONFIG_CONFIRM_EVAL_SET_MODE, CONFIG_CONFIRM_EVERY_K_EVALS, CONFIG_CONFIRM_IMPROVEMENT_EPS; best_search_x/f, best_confirm_x/f; official best = confirm-validated; final eval uses best_confirm_x on full_pool. Especially recommended for random search, DE, CMA-ES, PSO, surrogate methods (§2.2, §3.2, §9, §9.2, method table).
- **Output:** deltas_best.json, zero_order_state.json (method, n_evals_used, max_evals, remaining_evals, best_x/best_f/best_eval_id, seeds, eval_set_mode, indices_hash, history path, final_full_pool_f; when multi-fidelity: best_search_x/f, best_confirm_x/f, confirm_schedule), evaluation_indices.json when applicable, **final_eval.json** when CONFIG_RUN_FINAL_FULL_POOL_EVAL=y (best_x or best_confirm_x); processor_best.py once at end.
