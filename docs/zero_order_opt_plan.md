...
- **x:** list of `d` floats in **canonical cluster order** (`sorted(cluster_ids, key=int)` from initial deltas). Same list↔dict conversion as GA: `evolution/ga_driver._deltas_list_to_dict` / `_deltas_dict_to_list`.

### 1.2 How to evaluate f(x) (shared pipeline only; no re-implementation)

All evaluations go through the **shared** evaluation path. Zero-order is **not allowed** to re-implement these steps; it must call the shared evaluator (see §3.1, §5). The shared path reuses the existing logic from `evolution/ga_driver.py` (refactored into a single `_evaluate_x` or `evolution/objective.py` used by **both** GA and zero-order). Do **not** stop or restart SGLang per eval.
...
- **Plan rule:** Reuse runner/SGLang decoding settings where they exist. Add if missing: **CONFIG_EVAL_DETERMINISTIC** (bool, default y), **CONFIG_INFERENCE_SEED** or **CONFIG_SGLANG_SEED** (int); optionally CONFIG_TEMPERATURE, CONFIG_TOP_P. When CONFIG_EVAL_DETERMINISTIC=y, force temperature=0, top_p=1, and fixed seed in the runner config used for evaluation.
- **skopt:** **skopt is allowed only if** CONFIG_EVAL_SET_MODE is fixed_minibatch **and** CONFIG_EVAL_DETERMINISTIC=y. If determinism is off, **disable or forbid** skopt at config load or dispatcher (surrogates get confused by stochastic objectives).
...
### 3.1 Shared objective evaluator (mandatory refactor; no divergence)

- **Refactor requirement:** Refactor `ga_driver._evaluate_individual` so that **both** GA and zero-order use a **single shared** evaluation path. Extract a core function used by both:
  - **Option A:** `evolution/objective.py` exposes `evaluate_objective(...)` and a lower-level `_evaluate_x(x_list, cluster_ids, evaluation_indices, cfg, tokenizer, server_holder, out_dir, sglang_gpu_id, pre_generated_processor_path=None) -> (float, results)`. GA’s `_evaluate_individual` builds the list from the individual and calls `_evaluate_x`; zero-order calls `evaluate_objective` (or EvalContext wraps it).
  - **Option B:** Keep the core in `ga_driver` as `_evaluate_x(...)` and have `evolution/objective.py` (or EvalContext) call it. In both cases, **zero-order is not allowed to re-implement** generate-processor, SGLang lifecycle, runner config, training set, or correctness; it must call this shared evaluator only.
...
### 3.2 EvalContext and prefetch API (always prefetch when possible)

- **Module:** `evolution/eval_context.py` (new).
...
- **Requirement:** Population/batch methods (differential_evolution, CMA-ES, Optuna batch, SMAC batch) **must** call `evaluate_batch(xs)` so that prefetch runs for every transition; no ad-hoc loops that bypass this.
...
- **2-GPU enforcement:** The zero-order path **uses the same 2-GPU enforcement as DEAP**. At entry, call **`_validate_2_gpus()`** (from `evolution/ga_driver` or a shared helper). That helper enforces:
  ...
  - **If set:** Parse tokens; require all numeric; require **`len(tokens) == 2`** and **`tokens[0] != tokens[1]`**. If not, raise with the repo’s standard messages (see `docs/2XGPU_pod_plan.md` §2). Refactor `_validate_2_gpus()` into a shared module if needed so both GA and zero-order call the same logic.
- **Role of optimize_driver:** Load config (validate_ga=False when method != deap), resolve pool and **fixed minibatch** indices, load tokenizer, **start SGLang once** (same as GA, using the same `SGLangServer` and `sglang_gpu_id` from `_validate_2_gpus()`), run GPU verification (UUID match for SGLang PID). Build EvalContext. Call the chosen optimizer’s `run(ctx, x0, bounds)`. Persist zero_order_state.json, deltas_best.json, **evaluation_indices.json** (fixed minibatch), final_eval.json when CONFIG_RUN_FINAL_FULL_POOL_EVAL=y; optionally generate processor_best.py **once at end**. Population/batch methods use `ctx.evaluate_batch(xs)` so prefetch is always used.
...
| differential_evolution | pop × iters | **Yes** — **must** use `evaluate_batch(population)` each step | Native | workers=1; updating="immediate". |
| cmaes | pop × iters | **Yes** — **must** use `evaluate_batch(candidates)` each step | Native | workers=1. |
| optuna_tpe | n_trials | **Yes** — batch suggestions with `evaluate_batch(trials)` when possible | suggest_float | TPE sampler. |
| smac | n_trials | **Yes** — batch suggestions with `evaluate_batch(trials)` when possible | ConfigurationSpace | smac.validate(incumbent). |
| tr_dfo | maxfev (BOBYQA/NEWUOA/Py-BOBYQA/PDFO) | **No prefetch** — backend calls objective internally | Bounds / clip | Inline generate-processor each eval. Prefer ask/tell TR-DFO API if available. |
| skopt | n_calls | **Yes** when next suggest known (ask/tell or batch) | Native | **Allowed only if** CONFIG_EVAL_DETERMINISTIC=y. Surrogate: fixed minibatch + deterministic. |
| hybrid (optional) | global then local | Per phase: global uses evaluate_batch; local TR-DFO no prefetch | — | **(global) DE/CMA-ES/Optuna-TPE → (local) TR-DFO** using remaining budget. |

- **Prefetch alignment:** (1) **Population methods (DE, CMA-ES):** **must** call `ctx.evaluate_batch(population)` each step so prefetch runs for every transition. (2) **Optuna/SMAC:** batch-suggest when possible and evaluate via `evaluate_batch(trials)`. (3) **SPSA / RandomDirection_2pt:** evaluate the ± perturbation pair via **`evaluate_batch([x_plus, x_minus])`** each iteration for maximal prefetch. (4) **TR-DFO:** backend typically hides next x; **prefetch is not achievable** — state explicitly in implementation; inline generate-processor each eval. Prefer an ask/tell TR-DFO API if available. (5) **grid_search:** enumerate grid in deterministic order; stream points in batches of CONFIG_GRID_BATCH_SIZE and call **`ctx.evaluate_batch(batch_xs)`** for maximal prefetch.
...
- **Serial evaluation only:** workers=1, n_jobs=1, parallel=False; no vectorized objective. **grid_search:** Build per-dimension 1D grid `vals = [GRID_LOW + k*GRID_STEP ... <= GRID_HIGH]` (float-safe); enumerate all points as Cartesian product over d dims (canonical cluster order); stream in batches of CONFIG_GRID_BATCH_SIZE; call **ctx.evaluate_batch(batch_xs)**. Respect max_evals: if total_combos > max_evals and not CONFIG_GRID_ALLOW_TRUNCATION, fail before starting; if truncation allowed, stop at max_evals and return best-so-far. **Differential evolution:** workers=1, updating="immediate"; evaluate via **ctx.evaluate_batch(population)**. **CMA-ES:** evaluate via **ctx.evaluate_batch(candidates)**. **Optuna/SMAC:** batch-suggest when possible and use **ctx.evaluate_batch(trials)**. **SPSA / RandomDirection_2pt:** each iteration use **ctx.evaluate_batch([x_plus, x_minus])**. **TR-DFO (PDFO/Py-BOBYQA):** method="bobyqa" or equivalent, options={"maxfev": ...}; no prefetch (inline generate-processor per eval). **SMAC:** ConfigurationSpace with Float(..., bounds); use smac.validate(incumbent) for cost. **skopt:** only when CONFIG_EVAL_DETERMINISTIC=y; use fixed minibatch.
...
- **Shared evaluator (mandatory refactor):** Refactor `ga_driver._evaluate_individual` so that a single **`_evaluate_x(...)`** (or `evolution/objective.evaluate_objective`) implements the full pipeline: generate-processor (or pre_generated_processor_path), SGLang lifecycle (start once, keep running, restart only on crash), _build_runner_config, _run_training_set, _run_correctness, aggregation. **Both** GA and zero-order call this; zero-order **must not** re-implement any of these steps.
...
- **evolution/eval_context.py** — EvalContext: cluster_ids, eval-set indices, server_holder ref, n_evals, max_evals, BudgetExceeded, best_x/best_f/best_eval_id, single prefetch future, **evaluate(x, next_x=None)** and **evaluate_batch(xs)**. Calls the shared objective only.
- **evolution/optimizers/<method>.py** — Each implements run(ctx, x0, bounds) -> OptimizerResult; returns ctx.best_x, ctx.best_f on early termination. Population/batch methods use ctx.evaluate_batch(xs). **evolution/optimizers/grid_search.py** — Build 1D grid per dim (float-safe), Cartesian product over d dims, stream batches of CONFIG_GRID_BATCH_SIZE, call ctx.evaluate_batch(batch_xs); respect max_evals and CONFIG_GRID_ALLOW_TRUNCATION; log and persist grid params (d, GRID_LOW/HIGH/STEP, n_points, total_combos, max_evals, truncation) in zero_order_state.json.
- **evolution/optimize_driver.py** — Entry for method != deap: **same _validate_2_gpus()** as GA, load config (validate_ga=False), resolve pool/eval indices, tokenizer, start SGLang once (same SGLangServer and pinning as GA), GPU verify, EvalContext, **dispatcher:** grid_search → evolution.optimizers.grid_search.run (and other method→run mappings), run optimizer, persist state and deltas_best.json, optional processor_best.py at end.

### 12.2 evolution/__init__.py and entry

- **run_evolution(cfg):** If cfg.optimization_method == "deap": run_ga_evolution(cfg). Else: run_zero_order_evolution(cfg) (lives in optimize_driver.py). **Single entry point** `evolve --config`; branching only inside run_evolution.

---

## 13. Implementation Checklist

1. **Kconfig:** Add CONFIG_OPTIMIZATION_METHOD (choice: deap, **grid_search**, random_search, spsa, random_direction_2pt, differential_evolution, cmaes, optuna_tpe, smac, tr_dfo, skopt [conditional on CONFIG_EVAL_DETERMINISTIC=y], optionally hybrid), CONFIG_EVAL_MINIBATCH_SIZE, CONFIG_EVAL_SEED, CONFIG_ZERO_ORDER_MAX_EVALS, CONFIG_DELTAS_BOUND_LOW/HIGH, CONFIG_EVAL_TIMEOUT_S, CONFIG_EVAL_MAX_RETRIES, CONFIG_EVAL_FAILURE_FITNESS, CONFIG_OPTIMIZER_SEED, CONFIG_RUN_FINAL_FULL_POOL_EVAL, **CONFIG_LLM_MAX_TOKENS**, CONFIG_EVAL_DETERMINISTIC (default y), cache options, **grid_search when method=grid_search: CONFIG_GRID_LOW, CONFIG_GRID_HIGH, CONFIG_GRID_STEP, CONFIG_GRID_MAX_COMBOS (default 20000), CONFIG_GRID_ALLOW_TRUNCATION (default n), CONFIG_GRID_BATCH_SIZE (default 64)**, method-specific options (CONFIG_TR_DFO_METHOD, etc.). Parser and validation: GA only when method=deap; zero-order when method!=deap; skopt forbidden unless CONFIG_EVAL_DETERMINISTIC=y; **grid_search requires CONFIG_EVAL_DETERMINISTIC=y and grid validation (GRID_STEP>0, GRID_LOW<GRID_HIGH, total_combos ≤ GRID_MAX_COMBOS, and if total_combos>max_evals then GRID_ALLOW_TRUNCATION must be y)**.
2. **EvolutionConfig:** Add fields for optimization_method, zero_order_max_evals, eval_minibatch_size, eval_seed, eval_timeout_s, eval_max_retries, eval_failure_fitness, deltas_bound_low, deltas_bound_high, optimizer_seed, run_final_full_pool_eval, **llm_max_tokens**, cache options, **when method=grid_search: grid_low, grid_high, grid_step, grid_max_combos, grid_allow_truncation, grid_batch_size**, etc. kconfig_loader fills them when method != deap. (Eval-set is always fixed minibatch; no eval_set_mode choice.)
3. **Shared objective:** Implement evolution/objective.py (or refactor ga_driver to expose _evaluate_x). evaluate_objective(x, ...) with optional pre_generated_processor_path; clip x in wrapper for minimizers; return f. Used by EvalContext and (if refactored) by GA.
4. **EvalContext:** evolution/eval_context.py. Cluster_ids, **fixed minibatch** indices (CONFIG_EVAL_MINIBATCH_SIZE, CONFIG_EVAL_SEED), server_holder, n_evals, max_evals, BudgetExceeded before next eval, best_x/best_f/best_eval_id on success only. Single prefetch future; evaluate(x, next_x=None): wait prefetch, submit prefetch for next_x if provided, call objective with pre_generated_processor_path. Failure handling, logging, optional cache (when CONFIG_EVAL_DETERMINISTIC=y), history JSONL.
5. **Optimizers:** evolution/optimizers/<method>.py. run(ctx, x0, bounds) -> OptimizerResult; serial evaluation only; on BudgetExceeded/library abort return ctx.best_x, ctx.best_f. Force workers=1, n_jobs=1, etc. **grid_search:** Create evolution/optimizers/grid_search.py; stream grid in batches (CONFIG_GRID_BATCH_SIZE), ctx.evaluate_batch(batch_xs); log and persist grid params in zero_order_state.json.
6. **optimize_driver:** _validate_2_gpus(), load config (validate_ga=False for method!=deap), **dispatcher:** add grid_search → evolution.optimizers.grid_search.run, resolve pool and **fixed minibatch** indices, tokenizer, start SGLang once, GPU verify, create EvalContext, run optimizer, persist zero_order_state.json (including grid params for grid_search), deltas_best.json, evaluation_indices.json, final_eval.json when applicable, processor_best.py once at end. Prefetch: population methods use ctx.evaluate_batch(xs); SPSA/2pt use evaluate_batch([x_plus, x_minus]); grid_search uses evaluate_batch(batch_xs); TR-DFO has no prefetch (inline generate-processor).
7. **run_evolution:** In evolution/__init__.py (or ga_driver + optimize_driver): if cfg.optimization_method == "deap": run_ga_evolution(cfg); else run_zero_order_evolution(cfg).
8. **CLI:** No change; `evolve --config` calls run_evolution(cfg).
9. **Dependencies:** scipy baseline; optional extras with clear “install …” on import failure.
...
- **2-GPU pod:** **Exactly 2 visible GPUs (distinct)** required. Zero-order uses the **same _validate_2_gpus()** as DEAP. **GPU-A** = first visible, **GPU-B** = second. Generate-processor: `env = os.environ`, uses **GPU-A** (`cuda:0` in parent). SGLang: **only** SGLang may override CVD — pinned to **GPU-B** via CVD shim in SGLangServer.start() (this repo's SGLang has no device flag); SGLang process sees one GPU and uses `cuda:0` internally. No other subprocess overrides CVD. See §5.
- **Prefetch / shared pipeline:** **evaluate_batch(xs)** always prefetches xs[i+1] while evaluating xs[i]. **evaluate(x, next_x=...)** prefetches when next_x is provided. **TR-DFO** has no prefetch (backend hides next x; inline generate-processor). GA and zero-order **must** use the same refactored pipeline; zero-order may not re-implement it.
- **Unified interface:** EvalContext (evaluate, evaluate_batch), optimizers run(ctx, x0, bounds); population/batch methods must use ctx.evaluate_batch(xs). Single entry point: only `evolve --config`; Kconfig validation: GA options only when method=deap, zero-order only otherwise.
...
- **Output:** zero_order_state.json, deltas_best.json, evaluation_indices.json (fixed minibatch), final_eval.json (when CONFIG_RUN_FINAL_FULL_POOL_EVAL=y), zero_order_history.jsonl, processor_best.py once at end. **Incremental save and Ctrl+C:** State is saved during the run and on SIGINT so that Ctrl+C then re-run **resumes from the same place** (§10).
